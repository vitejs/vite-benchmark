...
export class Bench {
  ...
  public async run() {
    ...
    for (const metric of resolvedMetrics) {
      console.log(colors.green(`Start running benchmark ${metric.key}`))
      switch (metric.key) {
        case 'devStart':
          await this.metricDevStart(metric.options)
          break
        case 'build':
      ...
      console.log(colors.green(`Finish running benchmark ${metric.key}`))
    }

    this.packUpload()
  }

  public async metricDevStart({
    cleanCache,
    type,
  }: {
    cleanCache: boolean
    ...
    const timeoutHandler = setTimeout(() => {
      ...
      console.log(colors.red(`Timeout for dev-start`))
    ...
    await this.prepareUpload(`dev-start-${type}-`)
    await this.dispose()
    if (cleanCache) {
  ...
  public async startServer({
    ...
    this.serveChild.stdout?.on('data', (data: Buffer) => {
      this.debugLog += data.toString()
    })
  ...
  public async prepareUpload(prefix: string) {
    ...
    await writeFile(
      path.resolve(UPLOAD_DIR_TEMP, `./${this.name}-${prefix}debug-log.txt`),
      stripAnsi(this.debugLog)
    )

    await copyFile(
      path.resolve(this.caseDir, './cpu.cpuprofile'),
      path.resolve(UPLOAD_DIR_TEMP, `./${this.name}-${prefix}cpu.cpuprofile`)
    )
  }
